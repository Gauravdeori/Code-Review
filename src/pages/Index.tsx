...
const Index = () => {
  ...
  const handleReview = useCallback(async () => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error("Python Backend Error");
      
      const data = await resp.json();
      
      const newAction: ReviewAction = {
        type: "dqn_diagnosis",
        label: data.detected_issue,
        description: "Diagnosed by PyTorch DQN",
        confidence: 0.95,
        severity: data.reward_score >= 0 ? "success" : "warning",
        reward: data.reward_score,
        details: data.explanation || "No explanation attached."
      };
      ...
      const newEpisode: Episode = {
        ...
        reward: data.reward_score,
        totalReward: data.reward_score + (episodes.length > 0 ? episodes[episodes.length - 1].totalReward : 0)
      };
      ...
      setStats({
        ...
        epsilon: Number(data.agent_epsilon.toFixed(3)),
        qTableSize: 1 // Representing a Neural network rather than table
      });
      
      toast.success(`Episode complete — Reward: ${data.reward_score.toFixed(2)}`);
    } catch (e) {
      toast.error("Failed to connect to Python backend. Is uvicorn running?");
      console.error(e);
    } finally {
      setIsReviewing(false);
    }
  }, [code, language, episodes]);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <HeaderBar stats={stats} onTrainBatch={handleTrainBatch} isTraining={isReviewing} />
